		...
		if(istype(T))
			if(!T.density)
				for(var/mob/living/M in T)
					if(tram.Find(M))	continue
					thomas_the_tank_engine(M, dir)
	for(var/obj/tram/floor/F in tram_floors)
...
/obj/tram/tram_controller/proc/thomas_the_tank_engine(var/mob/living/M, var/dir)
	playsound(M.loc, 'sound/weapons/smash.ogg', 50, 10)
	M.visible_message("<span class='warning'>[M] is hit by the tram!</span>", "<span class='danger'>The tram hits you!</span>")
	shake_camera(M, 10, 1)
	//Throw diagonally so they can get thrown out of the way.
	var/throwdir1 = dir
	var/throwdir2 = dir
